...
def generate_decl_wrapper(name, return_type, argnames, argtypes, accelerate):
    ...
    if name in WRAPPED_FUNCS:
        return ""
    # lsame and dcabs1 are missing from new Accelerate
    # Use old symbols that do not require a wrapper
...
def generate_file_wrapper(sigs, lib_name, accelerate, outdir):
    if lib_name == 'BLAS':
        preamble = [C_PREAMBLE, CPP_GUARD_BEGIN]
    elif lib_name == 'LAPACK':
        preamble = [C_PREAMBLE, LAPACK_DECLS, CPP_GUARD_BEGIN]
    wrappers = {}
    for sig in sigs:
